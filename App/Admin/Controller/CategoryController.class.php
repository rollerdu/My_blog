...
class CategoryController extends BaseController
{
    ...
    {
        $model = M('Category');
        $category = $model->limit($Page->firstRow.','.$Page->listRows)->order('id ASC')->select();
    ...
    public function add()
    {
        ...
        if (!IS_POST) {
			...
			/*原二级分类的
			$model = M('business')->select();
            $cates = getSortedCategory($model);
			*/
    ...
    public function update()
    {
        //默认显示添加表单
        if (!IS_POST) {
            $model = M('category')->find(intval(I('param.id')));
          
            $this->assign('cate',getSortedCategory(M('category')->select()));
            $this->assign('data',$model);

            //原二级分类的$this->assign('cates',getSortedCategory(M('business')->select()));
            $this->display();
        }
        if (IS_POST) {
            $model = D("Category");
    ...
    public function delete($id)
    {
        $model = M('category');
        //查询属于这个分类的文章
        $posts = M('article')->where('id='.$id)->select();
        if($posts){
