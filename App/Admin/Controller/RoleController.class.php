...
class RoleController extends BaseController {
	...
	public function index(){
        ...
        foreach($data as $key => $val){
            $member = M("member")->where(array('role_id'=>$val['id'],'status'=>1))->getField('username',true);
            $data[$key]['member'] = implode(',',$member);
        }
	...
	public function delete() {
		...
		$count=$member_model->where("role_id=$id")->count();
		if($count){
			$this->error("该角色已经有用户！");
		}else{
    ...
    public function authorize() {
        ...
        if (IS_POST) {
            if (is_array($_POST['menuid']) &&  count($_POST['menuid'])>0 ) {
				...
				$names = serialize($_POST['menuids']);//业务添加
				$auth_access_model->add(array("role_id"=>$roleid,"rule_name"=>$names,'type'=>'admin_business'));//业务添加
